...
export default function SiteShell({ children }: { children: ReactNode }) {
  ...
  const horizontalLogo = isDark ? cachepdfAssets.horizontalDark : cachepdfAssets.horizontalLight;
  const footerLogo = isDark ? cachepdfAssets.horizontalDark : cachepdfAssets.horizontalLight;
  ...
  return <div className={`site-shell min-h-screen overflow-x-clip bg-[#07090d] text-[#f5f7fa] ${isDark ? "theme-dark" : "theme-light-surface"}`}>
    <header className="site-header sticky top-0 z-50 border-b border-white/[0.08] bg-[#07090d]/95 backdrop-blur-xl">
      <div className="container flex h-[68px] items-center justify-between gap-4">
        <Link href="/" className="group flex shrink-0" aria-label="CachePDF home"><img src={horizontalLogo} alt="CachePDF by Nivaronix" className="h-7 w-auto max-w-[172px] object-contain object-left" /></Link>
    ...
    {children}
    <footer className="site-footer border-t border-white/[0.08] bg-[#05070a]"><div className="container grid gap-10 py-12 md:grid-cols-[1.1fr_1.6fr] md:py-16"><div><Link href="/" className="inline-flex" aria-label="CachePDF home"><img src={footerLogo} alt="CachePDF by Nivaronix" className="h-8 w-auto max-w-[210px] object-contain object-left" /></Link><p className="mt-4 max-w-sm text-sm leading-6 text-[#98a2b3]">The private PDF workbench. Open, work, and export supported PDF workflows directly in your browser.</p><p className="mt-7 font-mono text-[10px] uppercase tracking-[0.18em] text-[#5fd8f5]">Open. Work. Export. Nothing leaves.</p></div><div className="grid grid-cols-2 gap-8 sm:grid-cols-3"><div><p className="footer-label">Workbench</p><div className="mt-4 flex flex-col gap-3"><Link href="/tools" className="footer-link">Open PDF</Link><Link href="/private-pdf-tools" className="footer-link">Private PDF tools</Link><Link href="/how-it-works" className="footer-link">How it works</Link><Link href="/guides" className="footer-link">Guides</Link></div></div><div><p className="footer-label">About</p><div className="mt-4 flex flex-col gap-3"><Link href="/support" className="footer-link">Support CachePDF</Link><Link href="/advertise" className="footer-link">Advertise</Link><Link href="/editorial-policy" className="footer-link">Editorial policy</Link><Link href="/about" className="footer-link">About CachePDF</Link></div></div><div className="col-span-2 sm:col-span-1"><p className="footer-label">Product studio</p><img src={parentLogoExtended} alt="Nivaronix" className="mt-4 h-auto w-[155px] opacity-90" /></div></div></div><div className="border-t border-white/[0.07]"><div className="container flex flex-col justify-between gap-3 py-5 font-mono text-[10px] uppercase tracking-[0.13em] text-[#687386] sm:flex-row"><span>© 2026 CachePDF by Nivaronix</span><span>LOCAL PROCESSING · ORIGINALS UNTOUCHED</span></div></div></footer>
  </div>;
}
