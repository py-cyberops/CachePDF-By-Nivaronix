...
export default function SiteShell({ children }: { children: ReactNode }) {
  ...
  const horizontalLogo = isDark ? cachepdfAssets.horizontalLight : cachepdfAssets.horizontalDark;
  const footerLogo = isDark ? cachepdfAssets.horizontalLight : cachepdfAssets.horizontalDarkAlt;
  ...
  return <div className={`site-shell min-h-screen overflow-x-clip bg-[#07090d] text-[#f5f7fa] ${isDark ? "theme-dark" : "theme-light-surface"}`}>
    <header className="site-header sticky top-0 z-50 border-b border-white/[0.08] bg-[#07090d]/95 backdrop-blur-xl">
      <div className="container flex h-[68px] items-center justify-between gap-4">
        <Link href="/" className="group flex shrink-0 items-center gap-3" aria-label="CachePDF home"><img src={horizontalLogo} alt="CachePDF" className="h-7 w-auto max-w-[138px] object-contain object-left" /><span className="hidden items-center gap-1.5 sm:inline-flex"><span className="font-mono text-[8px] uppercase tracking-[0.14em] text-[#8591a3]">by</span><img src={parentLogo} alt="Nivaronix" className="h-[11px] w-auto opacity-85" /></span></Link>
