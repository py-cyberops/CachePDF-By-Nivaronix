...
import { loadBrowserPdf, renderPdfPageToPng } from "@/lib/pdfBrowser";
...
function saveText(text: string) {
  ...
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = "onepdf-ocr.txt";
  document.body.appendChild(anchor);
  anchor.click();
  anchor.remove();
  window.setTimeout(() => URL.revokeObjectURL(url), 350);
...
function stateCopy(tool: ToolDefinition) {
  if (tool.slug === "ocr-pdf") return { label: "Browser OCR", text: "Pages render on this device. The OCR language model may download once, but your document is not uploaded.", tone: "cyan" };
...
export default function ToolWorkbench() {
  ...
  async function makeImagePdf() {
    ...
    return { bytes: await document.save(), name: "onepdf-images.pdf", mime: "application/pdf", label: "Your PDF is ready" };
  }
  ...
  async function exportImages(file: File, pages: number[]) {
    ...
    for (let index = 0; index < pages.length; index += 1) {
      ...
      const blob = await renderPdfPageToPng(document, pageNumber, Number(exportScale));
      archiveEntries[`${basename}-page-${String(pageNumber).padStart(2, "0")}.png`] = new Uint8Array(await blob.arrayBuffer());
    }
    return { bytes: zipSync(archiveEntries, { level: 6 }), name: `${basename}-images.zip`, mime: "application/zip", label: "Your image archive is ready" };
  }
  ...
  async function runOcr(file: File, pages: number[]) {
    ...
    try {
      for (let index = 0; index < pages.length; index += 1) {
        ...
        const image = await renderPdfPageToPng(document, pageNumber, 1.8);
  ...
  async function runTool() {
    ...
    if (!isImplemented || !activeTool) { toast.message("This engine is not enabled yet", { description: "OnePDF keeps the workspace visible but does not claim processing support before it is ready." }); return; }
    ...
    try {
      ...
      if (isMerge) {
        ...
        setResult({ bytes: await output.save(), name: "onepdf-merged.pdf", mime: "application/pdf", label: "Your PDF is ready" }); toast.success("Your PDF is ready."); return;
      }
      ...
      if (activeTool.slug === "pdf-to-images") { setResult(await exportImages(primaryFile.file, selection)); toast.success("Your page images are ready."); return; }
      if (activeTool.slug === "ocr-pdf") { const text = await runOcr(primaryFile.file, selection); setOcrText(text); setOcrProgress("OCR complete"); toast.success("Local OCR complete."); return; }
      ...
      if (activeTool.slug === "split-pdf" || activeTool.slug === "extract-pages") {
        const next = await PDFDocument.create(); const copied = await next.copyPages(source, selection.map((page) => page - 1)); copied.forEach((page) => next.addPage(page)); output = { bytes: await next.save(), name: activeTool.slug === "split-pdf" ? "onepdf-split.pdf" : "onepdf-extracted-pages.pdf", mime: "application/pdf", label: "Your PDF is ready" };
      } else if (activeTool.slug === "reorder-pages") {
        const next = await PDFDocument.create(); const copied = await next.copyPages(source, pageOrder.map((page) => page - 1)); copied.forEach((page) => next.addPage(page)); output = { bytes: await next.save(), name: "onepdf-reordered.pdf", mime: "application/pdf", label: "Your reordered PDF is ready" };
      } else if (activeTool.slug === "delete-pages") {
        Array.from(new Set(selection)).sort((a, b) => b - a).forEach((page) => source.removePage(page - 1)); output = { bytes: await source.save(), name: "onepdf-pages-deleted.pdf", mime: "application/pdf", label: "Your PDF is ready" };
      } else if (activeTool.slug === "rotate-pages") {
        source.getPages().forEach((page, index) => { const pageNumber = index + 1; const directRotation = pageRotations[pageNumber] ?? 0; const batchRotation = selection.includes(pageNumber) ? Number(rotation) : 0; if (directRotation || batchRotation) page.setRotation(degrees((page.getRotation().angle + directRotation + batchRotation) % 360)); }); output = { bytes: await source.save(), name: "onepdf-rotated.pdf", mime: "application/pdf", label: "Your PDF is ready" };
      } else if (activeTool.slug === "add-page-numbers") {
        source.getPages().forEach((page, index) => drawPageNumber(page, index + 1)); output = { bytes: await source.save(), name: "onepdf-numbered.pdf", mime: "application/pdf", label: "Your PDF is ready" };
      } else if (activeTool.slug === "add-watermark") {
        if (!watermark.trim()) throw new Error("Enter watermark text before processing."); source.getPages().forEach((page) => drawWatermark(page, watermark.trim())); output = { bytes: await source.save(), name: "onepdf-watermarked.pdf", mime: "application/pdf", label: "Your PDF is ready" };
      } else if (activeTool.slug === "remove-pdf-metadata") {
        source.setTitle(""); source.setAuthor(""); source.setSubject(""); source.setKeywords([]); source.setCreator(""); source.setProducer(""); output = { bytes: await source.save(), name: "onepdf-metadata-removed.pdf", mime: "application/pdf", label: "Your PDF is ready" };
      ...
      <main className="bg-[#07090d]">
        ...
        <section className="container grid gap-7 py-10 lg:grid-cols-[minmax(0,1fr)_300px] lg:py-14">
          <div className="surface overflow-hidden"><div className="border-b border-white/[0.09] px-5 py-4 sm:px-7"><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-[#a7b2c1]">01 // Select document</p></div><div className="p-5 sm:p-7"><input ref={inputRef} type="file" className="hidden" accept={needsImages ? "image/png,image/jpeg,.png,.jpg,.jpeg" : "application/pdf,.pdf"} multiple={isMerge || needsImages} onChange={onFileChange} />
            {!files.length ? <div onDragOver={(event) => { event.preventDefault(); setDragging(true); }} onDragLeave={() => setDragging(false)} onDrop={onDrop} className={`drop-zone ${dragging ? "drop-zone-active" : ""}`}><span className="flex h-12 w-12 items-center justify-center rounded-[12px] border border-[#05c8f6]/25 bg-[#05c8f6]/10 text-[#05c8f6]"><UploadCloud className="h-6 w-6" /></span><h2 className="mt-5 font-display text-xl font-medium tracking-[-0.04em] text-white">{needsImages ? "Drop PNG or JPG images here" : isMerge ? "Drop PDF files here" : "Drop a PDF here"}</h2><p className="mt-2 text-sm leading-6 text-[#8794a6]">{needsImages ? "Create a PDF without sending image files away." : "Choose a file from this device. The original stays untouched."}</p><button type="button" onClick={() => inputRef.current?.click()} className="button-secondary mt-6">Browse files <FolderOpen className="h-4 w-4" /></button><p className="mt-4 font-mono text-[9px] uppercase tracking-[0.15em] text-[#697588]">Up to 100 MB per file · {needsImages ? "PNG / JPG" : "PDF"}</p></div> : <><div className="flex items-center justify-between gap-4"><div><p className="font-display text-lg font-medium tracking-[-0.04em] text-white">{files.length} {files.length === 1 ? "file selected" : "files selected"}</p><p className="mt-1 text-sm text-[#8794a6]">{isMerge ? "Use the arrows to set document order." : needsImages ? "Images are included in the displayed order." : "Review the controls before running the tool."}</p></div><button type="button" onClick={() => inputRef.current?.click()} className="button-ghost text-sm text-[#7adff7]"><FilePlus2 className="h-4 w-4" /> Add {isMerge || needsImages ? "files" : "file"}</button></div><div className="mt-6 space-y-2">{files.map((file, index) => <div className="file-row" key={`${file.name}-${index}`}><span className="text-[#697689]"><GripVertical className="h-4 w-4" /></span><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-[7px] border border-white/[0.1] bg-[#0a0e14] text-[#8ee7fb]">{file.kind === "image" ? <ImageIcon className="h-4 w-4" /> : <FileText className="h-4 w-4" />}</span><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium text-[#e4ebf3]">{file.name}</p><p className="mt-0.5 font-mono text-[10px] text-[#7b8798]">{formatBytes(file.size)}{file.pages ? ` · ${file.pages} ${file.pages === 1 ? "page" : "pages"}` : ""}</p></div>{(isMerge || needsImages) && <div className="hidden items-center gap-1 sm:flex"><button className="icon-button-small" onClick={() => moveFile(index, -1)} disabled={index === 0} aria-label="Move file up"><ArrowUp className="h-3.5 w-3.5" /></button><button className="icon-button-small" onClick={() => moveFile(index, 1)} disabled={index === files.length - 1} aria-label="Move file down"><ArrowDown className="h-3.5 w-3.5" /></button></div>}<button className="icon-button-small text-[#f3a1a1] hover:border-[#ef8585]/50" onClick={() => removeFile(index)} aria-label={`Remove ${file.name}`}><Trash2 className="h-3.5 w-3.5" /></button></div>)}</div></>}
