...
export default function ToolWorkbench() {
  ...
  async function exportImages(file: File, pages: number[]) {
    ...
    return { bytes: zipSync(archiveEntries, { level: 6 }), name: `${basename}-${imageFormat}-images.zip`, mime: "application/zip", label: `Your ${imageFormat.toUpperCase()} image archive is ready` };
  }
  ...
  async function splitIntoIndividualPdfs(file: File, pages: number[]) {
    ...
    return { bytes: zipSync(archiveEntries, { level: 6 }), name: `${basename}-split-pages.zip`, mime: "application/zip", label: "Your individual PDF archive is ready" };
  }
  ...
  async function runTool() {
    ...
    try {
      ...
      if (activeTool.slug === "split-pdf") { setResult(await splitIntoIndividualPdfs(primaryFile.file, selection)); toast.success("Your individual PDFs are ready."); return; }
      ...
      if (activeTool.slug === "extract-pages") {
        const next = await PDFDocument.create(); const copied = await next.copyPages(source, selection.map((page) => page - 1)); copied.forEach((page) => next.addPage(page)); output = { bytes: await next.save(), name: "onepdf-extracted-pages.pdf", mime: "application/pdf", label: "Your PDF is ready" };
