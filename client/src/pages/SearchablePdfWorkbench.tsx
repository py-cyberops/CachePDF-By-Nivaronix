...
export default function SearchablePdfWorkbench() {
  ...
  async function openFile(candidate?: File) {
    ...
    try {
      ...
      setFile(candidate); setPageCount(source.getPageCount()); setPageRule("all"); setResult(null); setDownloadName(""); setProgress("");
  ...
  async function createSearchablePdf() {
    ...
    setBusy(true); setProgress("Preparing local OCR"); setResult(null); trackCachePdfEvent("operation_started", { tool: "make-pdf-searchable" });
    ...
    try {
      ...
      const worker = await createWorker("eng", 1, { logger: (message) => { if (message.status) setProgress(`${message.status}${typeof message.progress === "number" ? ` · ${Math.round(message.progress * 100)}%` : ""}`); } });
      ...
      setResult(next); setDownloadName(next.name); setProgress("Searchable PDF ready"); trackCachePdfEvent("operation_completed", { tool: "make-pdf-searchable", output: "pdf" });
