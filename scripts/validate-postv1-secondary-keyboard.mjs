...
try {
  ...
  await client.send("Page.navigate", { url: "http://localhost:3000/tools/document-privacy-check" }); await pause(1500); await setFixture(client);
  const checkbox = await evaluate(client, '(() => { const target = document.querySelector("[role=checkbox]"); if (!target) return null; target.tabIndex = 1; document.body.tabIndex = 0; document.body.focus(); return { before: target.getAttribute("aria-checked"), label: target.textContent?.trim() || "cleanup selection" }; })()');
  await client.send("Page.bringToFront"); await client.send("Input.dispatchKeyEvent", { type: "keyDown", key: "Tab", code: "Tab", windowsVirtualKeyCode: 9 }); await client.send("Input.dispatchKeyEvent", { type: "keyUp", key: "Tab", code: "Tab", windowsVirtualKeyCode: 9 }); await pause(80);
  const checkboxFocus = await evaluate(client, '(() => { const target = document.querySelector("[role=checkbox]"); if (!target) return null; const style = getComputedStyle(target); return { active: document.activeElement === target, focusVisible: target.matches(":focus-visible"), outlineStyle: style.outlineStyle, outlineWidth: style.outlineWidth }; })()');
  if (!checkboxFocus?.active || !checkboxFocus.focusVisible || checkboxFocus.outlineStyle !== "solid" || Number.parseFloat(checkboxFocus.outlineWidth) < 2) throw new Error(`Privacy cleanup visible focus failed: ${JSON.stringify({ checkbox, checkboxFocus })}`);
  await press(client, " ", "Space", 32, " "); const changed = await evaluate(client, `document.querySelector("[role=checkbox]")?.getAttribute("aria-checked") !== ${JSON.stringify(checkbox.before)}`); if (!changed) throw new Error("Privacy cleanup Space activation failed."); results.push({ label: checkbox.label, ...checkbox, ...checkboxFocus, activated: changed }); results.push(await assertButtonActivation(client, 'document.querySelector(".density-control button[title=\\"Compact spacing\\"]")', "Privacy Check compact density"));
